...
- **zsh 호환성**: 모든 스크립트가 zsh와 bash 양쪽에서 모두 작동
- **크로스 쉘 스크립트 경로 감지**: `BASH_SOURCE`와 `${(%):-%x}` 자동 선택
- **크로스 쉘 문자열 처리**: bash `${var,,}`와 zsh `${(L)var}` 호환 구현

### 개선/Improved

- **향상된 쉘 호환성**: macOS 기본 bash 3.2와 최신 zsh 5.9 모두 지원
